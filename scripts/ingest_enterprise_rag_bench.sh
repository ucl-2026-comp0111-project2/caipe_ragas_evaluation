...
uv run python3 -m ragas_eval.enterprise_rag_bench_ingest --rag-url https://rag.caipe.homelab --oidc-token-url "https://keycloak.caipe.homelab/realms/caipe/protocol/openid-connect/token" --oidc-client-id "${CLIENT_ID}" --oidc-client-secret "${CLIENT_SECRET}" --prioritize-reference --limit-per-source 1000 --insecure "$@"
